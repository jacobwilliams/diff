...
!*****************************************************************************************
	...
	! numbers.
	eta=digits(1.0_wp) - 1
	inf=-minexponent(1.0_wp) - 2
	sup=maxexponent(1.0_wp) - 1
...
130 n = n+1
	...
	if (init == 3) then
	! calculate coefficients for differentiation formulae and 
	! neville extrapolation algorithm
	  if (iord == 1) then
		beta=2.0_wp
	  ...
	  else if ((method /= 2 .and. iord == 1) .or. (method == 2 .and. iord == 2)) then
		e(1) = 10.0_wp
		e(2) = 16.0_wp
		e(3) = 20.36_wp
		e(4) = 23.0_wp
